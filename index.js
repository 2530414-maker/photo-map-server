// index.js — 최소 변경 최종본 (업로드 인증 없음 + 운영자 기능)
// npm i (Render가 자동 설치) — package.json에 firebase-admin 포함됨
...
// firebase-admin은 운영자 승인/취소에만 필요(없으면 해당 라우트에서 500 안내)
let admin = null;
try { admin = require("firebase-admin"); } catch (_) {}

const app = express();
app.use(cors());
app.use(express.json({ limit: "30mb" })); // 이미지 Base64 넉넉히
  ...
  try {
    ...
    req.user = {
      ...
      name:
        decoded.name ||
        decoded.displayName ||
        (decoded.email ? decoded.email.split("@")[0] : "익명"),
    };
    next();
  } catch (e) {
...
app.get(
  ...
  (req, res) => {
    res.json({ uid: req.user?.uid || null, isAdmin: ADMIN_UIDS.has(req.user?.uid), ADMIN_UIDS: [...ADMIN_UIDS] });
  }
);
